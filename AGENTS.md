...
│  ┌──────────┐                                                          │
│  │ DESKTOP  │                                                    ┌─────┴──────┐
│  │ (normal) │────────────────── Super+L ────────────────────────→│  LOCKED    │
│  │          │←────── unlock ─────────────────────────────────────│  (visible) │
│  └──────────┘                                                    │            │
│                                                                  │ FluxItem   │
│                           ┌──────────────────────────┐           │ running    │
│                           │ idleTimer (3s) fires     │           │ UI opacity │
│                           │ no input detected        │           │ = 1.0      │
│                           └───────────┬──────────────┘           └──────┬──────┘
│                                       │                                │
│                                       │  idle 3s timeout               │  idleTimer
│                                       ▼                                │  fires
│                            ┌──────────────────────┐                    │
│                            │      IDLE-HIDDEN     │←───────────────────┘
│                            │                      │
...
│                           ┌───────┴──────┐
│                           │              │
│                    mouse/keyboard    input idle
│                    (fade in UI)      (no input)
│                           │              │
│                           ▼              ▼
│                    ┌──────────────┐  ┌──────────────────────┐
│                    │   FADE IN   │  │ tetap IDLE-HIDDEN     │
│                    │  (visible)  │  │ (reset timer kembali) │
│                    └──────┬──────┘  └──────────────────────┘
│                           │
│                           ▼
│                    ┌──────────────┐
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  LockContext.qml (PAM)         LockScreen.qml (common)                  │
│  ┌──────────────────────┐      ┌──────────────────────┐                │
│  │  tryUnlock()         │      │                      │                │
│  │  targetAction        │─────→│  LockSurface.qml     │                │
│  │  unlockInProgress    │      │  ┌─────────────────┐ │                │
│  │  unlocked(action)    │      │  │  FluxItem       │ │                │
│  └──────────────────────┘      │  │  (z:-1)         │ │                │
│                                 │  │                 │ │                │
│  GlobalStates.qml               │  │  Timer(16ms)    │ │                │
│  ┌──────────────────────┐      │  │  idleTimer(3s)  │ │                │
│  │  screenLocked        │      │  │                 │ │                │
│  │  screensaverActive   │      │  │  Toolbar         │ │                │
│  └──────────────────────┘      │  │  passwordBox     │ │                │
│                                 │  └─────────────────┘ │                │
│  Config.options.fluid.*         └──────────────────────┘                │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
│                                                                         │
│  [Lock]                                                                 │
│    FluxItem.running = true                                             │
│    toolbarOpacity → 1 (NumberAnimation 200ms)                          │
│    idleTimer.start()                                                   │
│                                                                         │
│  [Idle 3s timeout]                                                     │
│    toolBar properties → Behavior turun ke 0 opacity                    │
│    FluxItem tetap running                                              │
│    (hanya fluid background visible)                                    │
│                                                                         │
│  [User input saat IDLE-HIDDEN]                                         │
│    idleTimer.reset()                                                   │
│    toolbarOpacity → 1 (NumberAnimation)                                │
│                                                                         │
│  [Unlock via password benar]                                           │
│    FluxItem.opacity → 0 (NumberAnimation 600ms)                       │
│    Timer 16ms stop                                                    │
│    Qt.callLater(() => context.unlocked(action)) setelah 600ms         │
│    (kalau Normal unlock: langsung ke desktop, skip fade)              │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
- 2026-06-24: Qt 6.11 cleanup ordering bug: `releaseResources()` dipanggil dari item destructor setelah GL context di-destroy. Mitigasi: `aboutToQuit` flag + `releaseResources()` leaks GPU resources (OS reclaims) instead of destructing with invalid GL context.
